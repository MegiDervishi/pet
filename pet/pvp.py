...
class FquadPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return [passage, '. Question: ', question, '? Est-ce ', answer, '?', self.mask, '.'], []
        if self.pattern_id == 1:
            return [passage, '. Question: ', question, '? La réponse correcte est-elle "', answer, '"?', self.mask, '.'], []
        if self.pattern_id == 2:
            return [passage, '. A partir du passage precedent, ', question, '? Est-ce que "', answer, '" est une bonne réponse?',
                    self.mask, '.'], []
        if self.pattern_id == 3:
            return [passage, question, '- [', self.mask, ']', answer], []
    ...
    def verbalize(self, label) -> List[str]:
        #if self.pattern_id == 3:
        #    return ['Faux'] if label == "0" else ['Vrai']
        return FquadPVP.VERBALIZER[label]
...
class IquadPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return [passage, '. Domanda: ', question, '? Può essere ', answer, '?', self.mask, '.'], []
        if self.pattern_id == 1:
            return [passage, '. Domanda: ', question, '? La risposta correta è "', answer, '"?', self.mask, '.'], []
        if self.pattern_id == 2:
            return [passage, '. Dal messagio precedente, ', question, '? La risposta "', answer, '" è giusta?',
                    self.mask, '.'], []
        if self.pattern_id == 3:
            return [passage, question, '- [', self.mask, ']', answer], []
    ...
    def verbalize(self, label) -> List[str]:
        #if self.pattern_id == 3:
        #    return ['Falso'] if label == "0" else ['Vero']
        return IquadPVP.VERBALIZER[label]
