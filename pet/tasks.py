...
class QuadProcessor(DataProcessor):
    """Processor for the (F/I)QuAD data set."""
    unlabeled = []

    def get_train_examples(self, data_dir):
        return self._create_examples_train(os.path.join(data_dir, "unlabeled.json"), "unlabeled")

    def get_dev_examples(self, data_dir):
        return self._create_examples_val(os.path.join(data_dir, "valid.json"), "dev")

    def get_test_examples(self, data_dir):
        return self._create_examples_val(os.path.join(data_dir, "test.json"), "test")

    def get_unlabeled_examples(self, data_dir):
        raise RuntimeError("Method implemeted for (f/i)quad in get_train_examples.")

    def get_labels(self):
        return ["0", "1"]

    
    def _create_examples_train(self, path: str, set_type: str) -> List[InputExample]:
        
        examples_good = self._create_examples_good(path, set_type)
        examples_bad = self._create_examples_bad(examples_good, path, set_type)
        
        train_good, train_bad = [], []
        
        k = 128

        idx = random.choices(list(range(0, len(examples_good))), k=k)
        
        for q_id in idx:
            prev = examples_good[q_id].guid.split("-")
            guid = 'train-'+'-'.join(prev[1:])
            examples_good[q_id].guid = guid
            train_good.append(examples_good[q_id])

            prev2 = examples_bad[q_id].guid.split("-")
            guid2 = 'train-'+'-'.join(prev2[1:])
            examples_bad[q_id].guid = guid2
            train_bad.append(examples_bad[q_id])
        


        unlabeled_good = list(set(examples_good)-set(train_good))
        unlabeled_bad= list(set(examples_bad)-set(train_bad))
        
        self.train = train_good + train_bad
        random.shuffle(self.train)
        self.unlabeled = unlabeled_good + unlabeled_bad
        random.shuffle(self.unlabeled)
        print('printing:', len(self.unlabeled))
        return self.train, self.unlabeled
        

    def _create_examples_val(self, path: str, set_type: str) -> List[InputExample]:
        
        examples_good = self._create_examples_good(path, set_type)
        examples_bad = self._create_examples_bad(examples_good, path, set_type)
        examples = examples_good + examples_bad
        random.shuffle(examples)
        return examples
        
    
    @staticmethod
    def _create_examples_bad(examples_good: List[InputExample], path: str, set_type:str) -> List[InputExample]:
        
        #pas = dict((i, []) for i in range(3166))
        pas = {}
        for i in examples_good:
            meta = i.meta
            if meta['passage_idx'] in pas:
                pas[meta['passage_idx']].append(meta['answer'])
            else:
                pas[meta['passage_idx']] = [meta['answer']]
        
        wrong_examples = []
        
        for val in examples_good:
            text_a = val.text_a
            text_b = val.text_b

            label="0"

            set_type = val.guid.split("-")[0]
            passage_idx = val.meta["passage_idx"]
            question_idx = val.meta["question_idx"]

            prev_answer = val.meta["answer_idx"]
            answer_idx = prev_answer - 2 * (prev_answer % 2) + 1

            possible_answers = list(set(pas[passage_idx]) - set([str(val.meta["answer"])]))
            if len(possible_answers) == 0:
                print(passage_idx)
                print(pas[passage_idx])
                print(val.meta['answer'])
                input()
            ...
            idx = [passage_idx, question_idx, answer_idx]
            example = InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, meta=meta, idx=idx)
    ...
    @staticmethod
    def _create_examples_good(path: str, set_type: str) -> List[InputExample]: 
        examples = []
        global_answers = 0
        global_questions= 0
        ...
        #check if has to specify it is tf8 character 
        with open(path, 'r', encoding='utf8') as f:
            data = json.load(f)
            datas = data['data']
            #print(len(datas))
            for line_idx, line in enumerate(datas):
                paragraphs = line['paragraphs']
                #print(len(paragraphs))
                #input()

                for real_paragraph_idx, ps in enumerate(paragraphs):
                    text = ps['context']
                    questions = ps['qas']

                    indices = set([ qs['id'] for qs in questions])
                    check_ans = set([ qs['answers'][0]['text'] for qs in questions ])
                    ...
                    passage_idx = global_passage_idx + 1
                    global_passage_idx = passage_idx

                    seen = set()
                    #print(len(questions))
                    for q_idx, qs in enumerate(questions):
                        if qs['id'] in seen:
                            continue
                        seen.add(qs['id'])
                        question = qs['question']
                        answers = qs['answers'][0]['text']
                        answer_idx = global_answers + random.randint(0,1)
                        global_answers += 2

                        question_idx = global_questions
                        global_questions += 1
                        ...
                        idx = [passage_idx, question_idx, answer_idx]
                        example = InputExample(guid=guid, text_a=text, text_b=question, label="1", meta=meta, idx=idx)
                        examples.append(example)

                    if len(examples) > cutoff:
                        break
                if len(examples) > cutoff:
                    break
...
TASK_HELPERS = {
    ...
    "fquad":task_helpers.MultiRcTaskHelper,
    "iquad":task_helpers.MultiRcTaskHelper,
}
...
def load_examples_quad_train(task, data_dir: str, set_type: str, *_, num_examples: int = None,
                  num_examples_per_label: int = None, seed: int = 42, num_examples_unlabel: int = None) -> List[List[InputExample]]:
    ...
    assert (set_type == TRAIN_SET), "set type must be TRAIN_SET"
    ...
    examples, examples_unlabel  = processor.get_train_examples(data_dir)
    for example in examples_unlabel:
    ...
    examples_unlabel = _shuffle_and_restrict(examples_unlabel, num_examples_unlabel, seed)

    label_distribution = Counter(example.label for example in examples)
    logger.info(f"Returning {len(examples)} {set_type} examples with label dist.: {list(label_distribution.items())}")

    label_distribution = Counter(example.label for example in examples_unlabel)
    logger.info(f"Returning {len(examples_unlabel)} unlabeled examples with label dist.: {list(label_distribution.items())}")
    input()

    return [examples, examples_unlabel]
